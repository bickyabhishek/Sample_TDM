...
def create_tree_index(documents,user_query):
    embeddings = initiate_google_embeddings()
    ...
    chain = RetrievalQA.from_chain_type(
        llm=initiate_google_chat_generative_ai(),
        retriever=retriever,
        chain_type="stuff"
    )
    return chain
...
def main():
    ...
    if st.button("Generate Tree Index"):
        ...
        if documents and user_query:
            st.session_state.chain = create_tree_index(documents,user_query)
    ...
    if "chain" in st.session_state:
        ...
        if st.button("Generate Response"):
            try:
                ...
                st.text_area("Response:", response['result'], height=200)
            except Exception as e:
