    ...
    if prompt_file_uploader:
        ...
        if st.button("Generate Synthetic Data"):
            try:
                ...
                if os.path.exists(os.path.join(constants.ROOT_PATH, constants.GENERATED_EXCEL_FOLDER_PATH, excel_file_name)):
                    excel_data = pd.ExcelFile(os.path.join(constants.ROOT_PATH, constants.GENERATED_EXCEL_FOLDER_PATH, excel_file_name))
                    ...
                    with st.container(border=True):
                        ...
                        with col1:
                            df1 = pd.read_excel(os.path.join(constants.ROOT_PATH, constants.GENERATED_EXCEL_FOLDER_PATH, excel_file_name), sheet_name='Header',  engine='openpyxl')
                            st.write(f"Displaying Sheet: {'Header'}")
                            st.dataframe(df1)

                        with col2:
                            df2 = pd.read_excel(os.path.join(constants.ROOT_PATH, constants.GENERATED_EXCEL_FOLDER_PATH, excel_file_name), sheet_name='Line',  engine='openpyxl')
